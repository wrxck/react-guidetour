...
import { Children, Fragment, cloneElement, isValidElement, useCallback, useEffect, useMemo, useReducer, useRef } from "react";
import { Fragment as Fragment$1, jsx, jsxs } from "react/jsx-runtime";
import { createPortal } from "react-dom";
import { deepmerge } from "deepmerge-ts";
import { createPopper } from "@popperjs/core";

//#region src/literals.ts
...
//#endregion
//#region src/components/Floater/Arrow.tsx
function FloaterArrow(props) {
	const { arrow, arrowRef, placement, styles } = props;
	const { arrow: { base, color, display, position, size } } = styles;
	const arrowStyles = {
		display,
		position
	};
	...
	let x = base;
	let y = size;
	if (placement.startsWith("top")) points = `0,0 ${x / 2},${y} ${x},0`;
	else if (placement.startsWith("bottom")) points = `${x},${y} ${x / 2},0 0,${y}`;
	...
	if (arrow) return /* @__PURE__ */ jsx("span", {
		ref: arrowRef,
		style: {
			color,
...
//#endregion
//#region src/components/Floater/CloseButton.tsx
...
//#endregion
//#region src/components/Floater/Container.tsx
function FloaterContainer(props) {
	const { content, footer, onClick, open, positionWrapper, showCloseButton, styles, title } = props;
	const output = { content: isValidElement(content) ? content : /* @__PURE__ */ jsx("div", {
		className: "__floater__content",
		style: styles.content,
		children: content
	}) };
	if (title) output.title = isValidElement(title) ? title : /* @__PURE__ */ jsx("div", {
		className: "__floater__title",
		style: styles.title,
		children: title
	});
	if (footer) output.footer = isValidElement(footer) ? footer : /* @__PURE__ */ jsx("div", {
		className: "__floater__footer",
		style: styles.footer,
		children: footer
	});
	if ((showCloseButton || positionWrapper) && !is.boolean(open)) output.close = /* @__PURE__ */ jsx(FloaterCloseButton, {
		onClick,
		styles: styles.close
	});
...
//#endregion
//#region src/components/Floater/index.tsx
function Floater(props) {
	const { component, content, floaterRef, hideArrow, id, onClick: closeFn, placement, status, styles } = props;
	const style = useMemo(() => {
		const { arrow: { size }, floater, floaterCentered, floaterClosing, floaterOpening, floaterWithComponent } = styles;
		let element = { ...floater };
		if (!hideArrow) {
			if (placement.startsWith("top")) element.padding = `0 0 ${size}px`;
			else if (placement.startsWith("bottom")) element.padding = `${size}px 0 0`;
			else if (placement.startsWith("left")) element.padding = `0 ${size}px 0 0`;
			else if (placement.startsWith("right")) element.padding = `0 0 0 ${size}px`;
		}
		if (status === STATUS.CLOSING) element = {
			...element,
			...floaterClosing
		};
		if (status === STATUS.OPENING || status === STATUS.OPEN) element = {
			...element,
			...floaterOpening
		};
		if (placement === "center") element = {
			...element,
			...floaterCentered
		};
	...
	if (component) {
		...
		output.content = isValidElement(component) ? cloneElement(component, componentProps) : component(componentProps);
	} else output.content = /* @__PURE__ */ jsx(FloaterContainer, {
		...props,
		content
	});
	if (status === STATUS.OPEN) classes.push("__floater__open");
	if (!hideArrow) output.arrow = /* @__PURE__ */ jsx(FloaterArrow, { ...props });
	if (!shouldRender) return null;
	return /* @__PURE__ */ jsx("div", {
		ref: floaterRef,
		id,
		style: { zIndex: styles.options.zIndex },
		children: /* @__PURE__ */ jsx("div", {
			...
			children: /* @__PURE__ */ jsxs("div", {
				...
				children: [output.content, output.arrow]
			})
		})
	});
}
...
const portalId = "react-floater-portal";
function canUseDOM() {
...
function log({ data, debug = false, title }) {
	...
	if (debug && title && data) {
		...
		if (Array.isArray(data)) data.forEach((d) => {
			if (is.plainObject(d) && d.key) logFn.apply(console, [d.key, d.value]);
			else logFn.apply(console, [d]);
		});
		else logFn.apply(console, [data]);
		console.groupEnd();
	}
}
...
function ReactFloaterPortal(props) {
	...
	const initialize = useCallback(() => {
		if (portalElement) node.current = is.string(portalElement) ? document.querySelector(portalElement) : portalElement;
		if (!portalElement || !node.current) {
			const portal = document.getElementById(portalId);
			if (portal) {
				const ids = portal.dataset.ids?.split(",").filter(Boolean) ?? [];
				if (!ids.includes(internalId) && internalId) ids.push(internalId);
	...
	useUnmount(() => {
		...
		try {
			if (node.current.id === portalId) {
				const remainingIds = (node.current.dataset.ids?.split(",") ?? []).filter((id) => id !== internalId);
	...
	if (node.current) {
		if (!hasChildren && !target && placement !== "center") return null;
...
//#endregion
//#region src/components/Wrapper.tsx
function FloaterWrapper(props) {
	const { childRef, children, id, isControlled, onClick, onMount, onMouseEnter, onMouseLeave, status, style, styles, wrapperRef } = props;
	useMount(() => {
		onMount();
	});
	let element = null;
	const mergedStyles = {
		...styles,
		...style,
		...isValidElement(children) ? children.props.style : void 0
	};
	...
	let wrapperProps = {
		"aria-describedby": [
			...
			STATUS.OPEN,
			STATUS.CLOSING
		].includes(status) ? id : void 0,
		style: mergedStyles
	};
	if (!isControlled) wrapperProps = {
		...wrapperProps,
		onClick,
		onMouseEnter,
		onMouseLeave
	};
	if (children) if (Children.count(children) === 1 && isValidElement(children) && children.type !== Fragment) element = is.function(children.type) ? /* @__PURE__ */ jsx("span", {
		ref: wrapperRef,
		id: wrapperId,
		...wrapperProps,
		children: cloneElement(Children.only(children), { innerRef: childRef })
	}) : cloneElement(Children.only(children), {
		id: wrapperId,
		ref: wrapperRef,
		...wrapperProps
	});
	else element = /* @__PURE__ */ jsx("span", {
		ref: wrapperRef,
		id: wrapperId,
		...wrapperProps,
		children
...
//#endregion
//#region src/modules/styles.ts
const defaultOptions = { zIndex: 100 };
...
//#endregion
//#region src/modules/useFloater.ts
function useFloater(props) {
	const { arrow: arrowElement, autoOpen = false, callback, children, component, content, debug = false, disableFlip = false, disableHoverToClick = false, event = "click", eventDelay = .4, footer, getPopper, hideArrow = false, id, modifiers, offset = 15, open, placement = "bottom", portalElement, showCloseButton = false, style, styles, target, title, wrapperOptions } = enhanceProps(props);
	...
	const eventDelayTimer = useRef(void 0);
	const internalId = useRef(randomId());
	const isMounted = useRef(false);
	const popperGeneration = useRef(0);
	const popperRef = useRef(void 0);
	const previousStatus = useRef(STATUS.INIT);
	const stateRef = useRef(state);
	const removeTransitionListener = useRef(void 0);
	const wrapperPopperRef = useRef(void 0);
	const wrapperStyles = useRef(void 0);
	const callbackRef = useRef(callback);
	const getPopperRef = useRef(getPopper);
	const initPopperRef = useRef(void 0);
	const propsRef = useRef(props);
	const targetElement = useRef(void 0);
	callbackRef.current = callback;
	getPopperRef.current = getPopper;
	propsRef.current = props;
	targetElement.current = () => {
		if (target) {
			if (is.domElement(target)) return target;
	...
	const currentEvent = useMemo(() => {
		if (event === "hover" && isMobile() && !disableHoverToClick) return "click";
		return event;
	...
	const currentStyles = useMemo(() => {
		...
		if (positionWrapper) {
			...
			if (!isIdle) wrapperCurrentStyles = nextStyles.wrapperPosition;
			else if (statusWrapper === STATUS.RENDER) wrapperCurrentStyles = wrapperPopperRef.current?.state.styles;
			nextStyles.wrapper = {
				...nextStyles.wrapper,
		...
		if (element) {
			...
			if (wrapperStyles.current) nextStyles.wrapper = {
				...nextStyles.wrapper,
				...
				if (!positionWrapper) {
					POSITIONING_PROPS.forEach((d) => {
						if (d === "position") wrapperStyles.current[d] = targetStyles[d];
						else wrapperStyles.current[d] = targetStyles[d];
					});
					nextStyles.wrapper = {
						...nextStyles.wrapper,
						...wrapperStyles.current
	...
	const updateState = useCallback((nextState, callback_) => {
	...
	const toggle = useCallback((forceStatus) => {
		...
		if (!is.undefined(forceStatus)) nextStatus = forceStatus;
		updateState({
			status: nextStatus,
	...
	const initPopper = useCallback(() => {
		...
		const element = targetElement.current();
		if (placement === "center") requestAnimationFrame(() => {
			...
			if (floaterRef.current) {
				...
				popperRef.current = createPopper(element, floaterRef.current, {
					...
					modifiers: [
						...
						},
						{
							...
							fn: ({ state: popperState }) => {
								const { elements: { arrow: stateArrow }, placement: statePlacement } = popperState;
								if (stateArrow) {
					...
					onFirstUpdate: (popperState) => {
						...
						if (placement !== popperState.placement) setTimeout(() => {
							popperRef.current?.forceUpdate();
						});
					}
				});
				if (getPopperRef.current && popperRef.current) getPopperRef.current(popperRef.current, "floater");
			} else if (stateRef.current.status === STATUS.RENDER)
 /* v8 ignore start -- @preserve */
			requestAnimationFrame(() => {
				if (isMounted.current && stateRef.current.status === STATUS.RENDER) initPopper();
			});
			else
 /* v8 ignore next 3 -- @preserve */
			...
			if (wrapperRef.current && !wrapperPopperRef.current && stateRef.current.positionWrapper) {
				...
				wrapperPopperRef.current = createPopper(element, wrapperRef.current, {
					...
					modifiers: [
						...
						{
							name: "offset",
							options: { offset: [0, wrapperOffset] }
						},
						{
							name: "flip",
							enabled: false
				...
				if (getPopperRef.current) getPopperRef.current(wrapperPopperRef.current, "wrapper");
			}
		}
	}, [
		disableFlip,
	...
	const cleanUp = useCallback(() => {
		...
		if (wrapperPopperRef.current) {
			...
			wrapperPopperRef.current = void 0;
		}
	}, []);
	initPopperRef.current = initPopper;
	const handleTransitionEnd = useCallback(() => {
		/* v8 ignore next 3 -- @preserve */
		if (wrapperPopperRef.current) wrapperPopperRef.current.forceUpdate();
		updateState({ status: stateRef.current.status === STATUS.OPENING ? STATUS.OPEN : STATUS.IDLE }, () => {
			if (callbackRef.current) callbackRef.current(stateRef.current.status === STATUS.OPEN ? "open" : "close", enhanceProps(propsRef.current));
		});
	}, [updateState]);
	const handleClick = useCallback(() => {
		...
		if (currentEvent === "click" || currentEvent === "hover" && positionWrapper) {
			...
			toggle(status === STATUS.IDLE ? STATUS.RENDER : void 0);
		}
	}, [
		currentDebug,
		currentEvent,
	...
	const handleMouseEnter = useCallback(() => {
		if (is.boolean(open) || isMobile() || currentEvent !== "hover") return;
		log({
			title: "mouseEnter",
			data: [{
				key: "originalEvent",
	...
	const handleMouseLeave = useCallback(() => {
		...
		if (currentEvent === "hover") {
			...
			if (!eventDelay) toggle(status === STATUS.CLOSING ? STATUS.IDLE : STATUS.CLOSING);
	...
	useUpdateEffect(() => {
		if (is.boolean(open)) toggle(open ? STATUS.RENDER : STATUS.CLOSING);
	}, [open, toggle]);
	useEffect(() => {
		const next = !!wrapperOptions?.position && !!target;
		if (stateRef.current.positionWrapper !== next) updateState({ positionWrapper: next });
	}, [
		wrapperOptions?.position,
		target,
		updateState
	]);
	useEffect(() => {
		...
		if (status === STATUS.IDLE && previousStatus.current !== STATUS.IDLE) {
			if (open || previousStatus.current === STATUS.INIT && autoOpen) toggle(STATUS.RENDER);
		}
		if (status === STATUS.RENDER && previousStatus.current !== STATUS.RENDER) {
			if (popperRef.current) popperRef.current.destroy();
			initPopper();
		}
		if (floaterRef.current && (status === STATUS.RENDER || status === STATUS.CLOSING) && previousStatus.current !== status) {
			removeTransitionListener.current?.();
			removeTransitionListener.current = once(floaterRef.current, "transitionend", handleTransitionEnd);
		}
...
function FloaterComponent(props) {
	const { arrowElement, arrowRef, childRef, children, component, content, currentPlacement, currentStyles, floaterRef, footer, handleClick, handleMouseEnter, handleMouseLeave, handleWrapperMount, hideArrow, id, internalId, open, portalElement, positionWrapper, showCloseButton, status, style, target, title, wrapperRef } = useFloater(props);
	const wrapper = /* @__PURE__ */ jsx(FloaterWrapper, {
		childRef,
		id: id ?? internalId,
		isControlled: is.boolean(open),
		onClick: handleClick,
	...
	return /* @__PURE__ */ jsxs(Fragment$1, { children: [/* @__PURE__ */ jsxs(ReactFloaterPortal, {
		hasChildren: !!children,
		internalId,
...
function ReactFloater(props) {
	...
	return /* @__PURE__ */ jsx(FloaterComponent, { ...props });
}
